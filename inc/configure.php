...
if ( isset( $_POST['s'] ) ) {
	...
	define( 'CONFIG_FILE_TEMP' , '../config.inc.php.tmp' );
		...
		switch ( $_POST['s'] ) {
			...
			case 'exist':
				if ( file_exists( '../config.inc.php' ) ) {
					throw new Exception( __( 'File <code>config.inc.php</code> already exists. Please remove it manually if you want me to create it.') );
				}
				break;


			/**
			 * Try to touch config.inc.php
			 */
			case 'touch':
			...
			case 'configure':
				...
				// Copy base
				file_put_contents( CONFIG_FILE_TEMP , str_replace( '%AUTOGENERATEDDATE%' , date('Y/M/d H:i:s') , file_get_contents( '../cfg/pimpmylog.config.php' ) ) ) ;
//				file_put_contents( CONFIG_FILE_TEMP , file_get_contents() , FILE_APPEND );
				...
				foreach( $logs as $log ) {
					...
					if ( function_exists( $get_config ) ) {
						$append = call_user_func( $get_config , $type , $file , $software , $counter );
						file_put_contents( CONFIG_FILE_TEMP , $append , FILE_APPEND );
					}
					else {
						@unlink( CONFIG_FILE_TEMP );
	...
	<div class="jumbotron" id="congratulations" style="display:none">
		<div class="container">
			...
			<?php
				...
				echo '<br/>';
				echo '<br/>';
				_e( 'You can manually adjust settings in the <code>config.inc.php</code> file.' );
				echo '<br/>';
